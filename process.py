...
def parse_depth_chart_html(html_content):
    ...
    if left_table:
        for row in left_table.find('tbody').find_all('tr', class_='Table__TR'):
            ...
            if position_span:
                # Extract only the position abbreviation (e.g., "QB", "RB")
...
def convert_to_csv(parsed_data):
    ...
    csv_rows = []
    # CSV Header
    csv_rows.append(['Formation', 'Position', 'Depth', 'PlayerName', 'PlayerURL', 'PlayerUID', 'InjuryStatus'])
...
if __name__ == "__main__":
    # Define the directory and filename to read
    input_directory = "team_tables"
    input_filename = "arizona-cardinals_depth_chart_offense.html" # You can change this to any offense file
    ...
    try:
        with open(input_filepath, 'r', encoding='utf-8') as f:
            html_content = f.read()
        print(f"Successfully loaded HTML from {input_filepath}")
    except FileNotFoundError:
        print(f"Error: The file '{input_filepath}' was not found. Please ensure it exists in the '{input_directory}' directory.")
    ...
    if html_content:
        ...
        if parsed_data:
            ...
            if xml_root is not None:
                ...
                output_xml_filename = input_filename.replace('.html', '.xml')
                output_xml_filepath = os.path.join(input_directory, output_xml_filename)
                with open(output_xml_filepath, "w", encoding="utf-8") as f:
                    f.write(xml_output)
                print(f"\nXML saved to {output_xml_filepath}")
            else:
                print("Failed to convert to XML.")

            # 3. Convert to CSV
            csv_output = convert_to_csv(parsed_data)
            if csv_output is not None:
                print("\n--- CSV Output ---")
                print(csv_output)
                
                # Optionally save to a file
                output_csv_filename = input_filename.replace('.html', '.csv')
                output_csv_filepath = os.path.join(input_directory, output_csv_filename)
                with open(output_csv_filepath, "w", encoding="utf-8", newline='') as f:
                    f.write(csv_output)
                print(f"\nCSV saved to {output_csv_filepath}")
            else:
